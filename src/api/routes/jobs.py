...
from src.api.deps import get_grade_store, get_job_store, get_mapper, get_question_store
...
def _run_batch(
    ...
    try:
        ...
        feat_matrix = extract_features(question_text, responses, reference, strictness)
        grades = mapper.predict_batch(feat_matrix)
        for i, (resp, grade_val) in enumerate(zip(responses, grades)):
            grade_store.insert({
                "job_id": job_id,
                "response_id": student_ids[i] if i < len(student_ids) else str(i),
                "predicted_grade": round(grade_val, 2),
                "features": {k: round(float(v), 4) for k, v in zip(FEATURE_NAMES, feat_matrix[i])},
                "model_version": "1.0",
            })
